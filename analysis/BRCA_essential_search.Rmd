---
title: "BRCA Essential Pair Search"
...
gene_alteration_rates = full_brca %>%
...
```{r}
total_samples = length(unique(full_brca$case_id))
full_brca_altered = full_brca %>%
...
possible_essential_pairs = gene_alteration_rates %>% 
  ...
  mutate(overlap_count = future_map2_int(gene_1,gene_2,
                                         find_overlap_count,
                                         alteration_data = full_brca_altered, 
                                         .progress=T)) %>%
  
  #make sure gene_1, gene_2 are characters to allow the next left_joins to work
  mutate(gene_1 = as.character(gene_1), gene_2 = as.character(gene_2)) %>%
